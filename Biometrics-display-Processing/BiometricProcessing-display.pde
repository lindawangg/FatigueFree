...
int PulseWindowHeight = 400;//512; 
int BPMWindowWidth = 180;
int BPMWindowHeight = 340;
boolean beat = false;    // set when a heart beat is detected, then cleared when the BPM graph is advanced
int gripStrength = 0;
//String threshold = "/data/Threshold1.exe";
...
// Scrollbar constructor inputs: x,y,width,height,minVal,maxVal
  ...
  zoom = 0.50;                               // initialize scale of heartbeat window
...
// GO FIND THE ARDUINO
  printArray(Serial.list());    // prin t a list of available serial ports
...
// DRAW THE PULSE WAVEFORM
  ...
  offset = map(zoom,0.5, 1, 150,0);// 0.5, 1, 150, 0               // calculate the offset needed at this scale
  for (int i = 0; i < RawY.length-1; i++) {      // move the pulse waveform by
    RawY[i] = RawY[i+1];                         // shifting all raw datapoints one pixel left
    float dummy = RawY[i] * zoom + offset;       // adjust the raw data to the selected scale
    ScaledY[i] = constrain(int(dummy),100,500);//44, 556   // transfer the raw data array to the scaled array
  }
...
// PRINT THE DATA AND VARIABLE VALUES
  ...
  text("Fatigue Free",100,30);     // tell them what you are
  text("HRV " + IBI + "mS",600,585);                    // print the time between heartbeats in mS
  text(BPM + " BPM",600,200);     // print the Beats Per Minute
  text(gripStrength - 900 + " Grip Strength", 245, 530);
  if (((restingBPM - BPM)*1000 + (1023 - gripStrength)*100 + score) > 15000)
  {
    text("Level 1 Fatigue detected", 400, 30);
  }
  //text("BPM SD " + SDBPM, 100, 80);
  text("Resting BPM " + restingBPM, 300, 80);
    
  //text(BPM + " BPM",245,200);
  //if (BPM < 65 && !activate) {
    //text("Threshold 1", 300, 30);
    //launch(threshold);
  //}
  //else {
    //sleepy.play();
